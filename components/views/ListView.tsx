...
export const ListView: React.FC<ListViewProps> = ({ type, setView, setSelectedArticle }) => {
  ...
  const handleCardClick = (itemId: string) => {
    if (!isVideo && setView && setSelectedArticle) {
      setView(ViewState.ARTICLE_DETAIL);
      setSelectedArticle(itemId);
    }
  };
  ...
  return (
    <div className="pt-28 pb-12 max-w-5xl mx-auto px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {items.map((item) => (
          <NeoCard key={item.id} hoverEffect className="group flex flex-col h-full overflow-hidden p-0" onClick={() => handleCardClick(item.id)}>
